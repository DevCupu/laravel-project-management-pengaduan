                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    @forelse ($pengaduans as $pengaduan)
                        <tr>
                            ...
                            <td class="px-4 py-4 whitespace-nowrap text-gray-800">{{ $pengaduan->judul }}</td>
                            <td class="px-4 py-4 whitespace-nowrap text-gray-600 max-w-xs truncate">
                                {{ Str::limit($pengaduan->isi, 40) }}</td>
                            ...
                            <td class="px-4 py-4 whitespace-nowrap flex gap-2">
                                <a href="{{ route('admin.pengaduan.show', $pengaduan) }}"
                                    class="inline-flex items-center px-3 py-1 text-xs font-semibold text-blue-600 hover:underline">
                                    <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                @if ($pengaduan->status !== 'selesai')
                                    <form method="POST" action="{{ route('admin.pengaduan.update', $pengaduan) }}"
                                        ...
                                        <button type="submit"
                                            class="inline-flex items-center px-3 py-1 text-xs font-semibold text-white bg-green-600 hover:bg-green-700 rounded transition">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="text-center py-8 text-gray-400">Belum ada pengaduan.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
